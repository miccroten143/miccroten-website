...
export default function CheckoutPage() {
  ...
  const shippingReady = !!shippingEstimate && !checkingShipping;
  const effectiveShipping = shippingEstimate?.cost ?? 0;
  const effectiveTotal = getSubtotal() - getDiscount() + getGst() + effectiveShipping;
    ...
    const checkShipping = async () => {
      ...
      try {
        const totalWeight = items.reduce(
          (total, item) =>
            total + ((item.product.weight_kg ?? 0.5) * item.quantity),
          0,
        );
  ...
  }, [selectedAddress, items, getSubtotal]);
  ...
  const handlePay = async () => {
    ...
    if (!shippingReady) {
      toast.error('Please wait for the delivery estimate to finish loading.');
      return;
    }
    setProcessing(true);
    setStep('processing');
    ...
    try {
      ...
      const order = await createOrder(
        ...
        items.map((i) => ({
          ...
          weight_kg: i.product.weight_kg ?? null,
          length_cm: i.product.length_cm ?? null,
          width_cm: i.product.width_cm ?? null,
          height_cm: i.product.height_cm ?? null,
        }))
      );
      ...
      // NEW — book the shipment with Shiprocket
      try {
        ...
        // Consider a background retry/admin alert here later.
      }
              ...
              {step === 'address' && (
                <motion.div key="address" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  <div className="bg-white rounded-xl shadow-md p-6">
                    ...
                    {checkingShipping && (
                      <div className="mt-4 p-4 bg-gray-50 rounded-lg flex items-center gap-3 text-sm text-gray-500">
                        <Truck className="h-5 w-5 animate-pulse" />
                        Calculating delivery estimate...
                      </div>
                    )}
                    {shippingEstimate && selectedAddress && !checkingShipping && (
              ...
              {step === 'payment' && (
                <motion.div key="payment" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  <div className="bg-white rounded-xl shadow-md p-6">
                    ...
                    <button onClick={handlePay} disabled={processing || !shippingReady} className="btn btn-primary w-full disabled:opacity-50">
                      <Lock size={16} className="mr-2" />
                      {processing
                        ? 'Processing...'
                        : shippingReady
                          ? `Pay ${formatINR(effectiveTotal)} via Razorpay`
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-md p-6 sticky top-20">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  {checkingShipping ? (
                    ...
                    <span className={effectiveShipping === 0 ? 'text-emerald-600' : ''}>
              ...
              <div className="flex justify-between border-t border-gray-100 pt-4 mt-4">
                ...
                <span className="text-xl font-bold text-primary-600">{shippingReady ? formatINR(effectiveTotal) : '—'}</span>
              </div>
...
function AddressForm({ onSave, onCancel }: { onSave: (data: Omit<Address, 'id' | 'user_id' | 'created_at' | 'updated_at'>) => void; onCancel: () => void }) {
