...
import {
  ...
  Pencil, CreditCard, Truck, Clock, Navigation,
} from 'lucide-react';
                          ...
                          <div className="flex justify-between items-start gap-3">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${order.payment_status === 'paid' ? 'bg-emerald-100 text-emerald-700' : order.payment_status === 'failed' ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}>{order.payment_status.toUpperCase()}</span>
                                {order.shipment_status && order.shipment_status !== 'pending' && (
                                  <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${order.shipment_status === 'delivered' ? 'bg-emerald-100 text-emerald-700' : order.shipment_status === 'cancelled' || order.shipment_status === 'rto' ? 'bg-rose-100 text-rose-700' : 'bg-blue-100 text-blue-700'}`}>{order.shipment_status.replace(/_/g, ' ').toUpperCase()}</span>
                                )}
                                {order.courier && (
                                  <span className="text-xs text-gray-500 flex items-center gap-1"><Truck size={12} /> {order.courier}</span>
                                )}
                                {order.estimated_delivery && (
                                  <span className="text-xs text-gray-500 flex items-center gap-1"><Clock size={12} /> ETA: {formatDate(order.estimated_delivery)}</span>
                                )}
                              </div>
                            </div>
                            <div className="text-right flex-shrink-0">
                              <p className="font-bold text-primary-600">{formatINR(order.total)}</p>
                              <span className="text-xs text-primary-600 font-medium flex items-center gap-1 justify-end mt-1"><Navigation size={12} /> Track</span>
                            </div>
                          </div>
                        </Link>
                      ))}
