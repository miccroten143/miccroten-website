...
export async function calculateShippingEstimate(
  ...
  if (error) {
    ...
    throw new Error(
      error.message || "Failed to calculate delivery estimate",
    );
  }

  if (!data?.success) {
    throw new Error(
      data?.error || "Delivery is not available for this pincode",
    );
  }

  return {
    cost: data.shipping?.cost ?? 0,
    estimatedDays: data.shipping?.estimatedDays ?? 0,
    estimatedDelivery:
      data.shipping?.estimatedDelivery ?? "Delivery date unavailable",
    courier: data.shipping?.courier ?? "Standard",
    available: data.shipping?.available ?? true,
  };
}
